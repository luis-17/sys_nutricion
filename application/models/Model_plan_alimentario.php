...
class Model_plan_alimentario extends CI_Model {
	...
	public function m_cargar_plan_alimentario($datos){
		...
		$this->db->select('ada.idatenciondietaalim, ada.valor, ada.idalimento',FALSE);
		$this->db->select('adaa.idatenciondietaalimalter, adaa.idalimento as idalimento_alter',FALSE);
		...
		$this->db->join('atencion_dieta_alim ada', 'ada.idatenciondietaturno = adt.idatenciondietaturno','left');
